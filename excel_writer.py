...
class _WorkbookEditor:
    ...
    def _setup_workbook(self) -> bool:
        if pathlib.Path.exists(pathlib.Path(self._args.excel)):
            ...
            self._set_subheading_alignment()
            self._write_headings()

            return True

    def _set_subheading_alignment(self) -> None:
        """
...
class ClinicallyRelevant:
    def __init__(self, data_frame: pd.DataFrame, args: argparse.Namespace):
        ...
        self.sheet: Worksheet = self._workbook[self._editor.clinical_sheetname]
    ...
    def _write_clinical_name_id(self):
        ...
        for i, (name, protein_id, concentration) in enumerate(
            ...
            self.sheet.cell(row=i + 3, column=1, value=name)
            self.sheet.cell(row=i + 3, column=2, value=protein_id)

            if int(concentration) != -1:
                self.sheet.cell(row=i + 3, column=3, value=concentration)
    ...
    def _write_clinical_data(self):
        ...
        start_col = self._editor.get_column_write_start(self.sheet.title)
        ...
        for i, (
            ...
            for j, row_contents in enumerate(
                self.sheet.iter_rows(min_row=3, min_col=2, max_col=2)
            ):
                row_index = j + 3
                ...
                if clinical_id == excel_id:
                    ...
                    self.sheet.cell(row=row_index, column=start_col + 4, value=ratio)
...
class AllProteins:
    def __init__(self, data_frame: pd.DataFrame, args: argparse.Namespace):
        self.editor = _WorkbookEditor(args)
        self.workbook: Workbook = self.editor.workbook
        self.sheet: Worksheet = self.workbook[self.editor.all_proteins_sheetname]
...
if __name__ == "__main__":
    ...
    data_frame: pd.DataFrame = create_intensity_dataframe(
        "/Users/joshl/PycharmProjects/MaxQuantAnalysis/data/c18/sdc/proteinGroups.txt"
    )
    ...
    args = argparse.Namespace(
        ...
        sdc=True,
        urea=False,
        input="data/c18/sdc/proteinGroups.txt",
        excel="data/refactor_class.xlsx",
        method="C18",
        experiment="SDC",
    )

    clinical = ClinicallyRelevant(data_frame, args)
